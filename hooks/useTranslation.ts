
import React, { createContext, useState, useContext, ReactNode, useCallback } from 'react';
// @google/genai-fix: Import TFunction to correctly type the translation function.
import i18next, { TFunction } from 'i18next';
import { initReactI18next, useTranslation as useI18nextTranslation } from 'react-i18next';

// --- DICTIONARIES ---
// The content of the JSON files is embedded directly to avoid module resolution errors.
const translationsEn = {
  "translation": {
    "appTitle": "Public Health AI Assistant",
    "welcomeTitle": "Hello! How can I assist you today?",
    "welcomeSubtitle": "I am an AI assistant from the Mukdahan Provincial Public Health Office.",
    "welcomeUploadSupported": "Supported formats: Images, Videos, and documents (.pdf, .docx, .txt, etc.)",
    "suggestedQuestions": [
      "What are the symptoms of dengue fever?",
      "I have a headache and a fever, what could it be?",
      "Book an appointment with a dentist for next Friday morning.",
      "Schedule my allergy pill for every day at 8 PM for the next 30 days.",
      "How do I apply for a health certificate?",
      "Create a 3-day healthy meal plan for me.",
      "Estimate the calories from a photo of my meal"
    ],
    "capabilityTitle": "My Capabilities",
    "capabilityChat": "Conversational AI",
    "capabilityChatDesc": "Ask me complex questions.",
    "capabilityImage": "Image Analysis",
    "capabilityImageDesc": "Analyze and discuss images.",
    "capabilityDoc": "File Q&A",
    "capabilityDocDesc": "Ask questions about uploaded images, videos, or documents.",
    "capabilityFood": "Food Analysis",
    "capabilityFoodDesc": "Estimate calories from a photo of your meal.",
    "capabilityVoice": "Voice Commands",
    "capabilityVoiceDesc": "Control the app with your voice.",
    "recommendSystem": "Recommend a healthy lunch menu for me",
    "recommendSystemPrompt": "Recommend a healthy lunch menu for me",
    "imageGenerationTriggers": [
      "generate image of",
      "create image of",
      "draw an image of",
      "make an image of"
    ],
    "imageEditingTriggers": [
      "edit image:",
      "edit this image:",
      "change image:",
      "change this image:",
      "add to image:",
      "remove from image:"
    ],
    "videoGenerationTriggers": [
      "generate video of",
      "create video of",
      "make a video of"
    ],
    "newChatTitle": "New Chat",
    "welcomeMessage": "Hello! I am your Public Health AI Assistant. How can I help you today?",
    "locationError": "Could not get your location. Please enable location services.",
    "toastNoActiveConversation": "No active conversation to export.",
    "toastExporting": "Exporting conversation...",
    "docConfirmTitle": "Start a new chat with this file?",
    "docConfirmMessage": "This will create a new conversation based on '{{fileName}}'.",
    "toastUnsupportedFile": "File '{{fileName}}' has an unsupported file type.",
    "toastUnsupportedFileForNewChat": "Cannot start a new chat. This file type is not supported.",
    "emergencyModalApiError": "Could not contact the emergency service API. Please check your connection or use the numbers below.",
    "emergencyModalLocationError": "Could not get your location for SOS. Please enable location services and try again.",
    "toastNewChat": "Started a new chat.",
    "toastReading": "Reading last message...",
    "toastNoMessage": "No message to read.",
    "toastDarkMode": "Switched to dark mode.",
    "toastLightMode": "Switched to light mode.",
    "toastCameraOn": "Camera opened.",
    "toastCameraOff": "Camera closed.",
    "toastAnalyzing": "Analyzing image...",
    "toastNotInLiveMode": "This command only works in Live mode.",
    "toastCameraNotOn": "Please open the camera first.",
    "tooltipSos": "Emergency (SOS)",
    "chat": "Chat",
    "live": "Live",
    "analyzer": "Analyzer",
    "hub": "News",
    "dashboard": "Dashboard",
    "map": "Map",
    "profile": "Profile",
    "nutrition": "Nutrition",
    "wellness": "Wellness",
    "pharmacy": "Pharmacy",
    "navGroupTalk": "AI Assistant",
    "navGroupTools": "Health Tools",
    "navGroupMe": "My Health",
    "navGroupInfo": "News",
    "tooltipPersona": "Set Persona / System Instruction",
    "tooltipExport": "Export Conversation",
    "tooltipTtsOn": "Text-to-Speech is ON",
    "tooltipTtsOff": "Text-to-Speech is OFF",
    "tooltipVoiceOn": "Voice Command is Listening",
    "tooltipVoiceOff": "Voice Command is OFF",
    "tooltipLanguage": "Switch Language",
    "tooltipTheme": "Toggle Theme",
    "tooltipFlashlight": "Flashlight",
    "tooltipElderlyMode": "Elderly Mode",
    "copyCode": "Copy",
    "errorOccurred": "An error occurred.",
    "clickToRetry": "Click to retry",
    "thinking": "Thinking...",
    "generatingImage": "Generating image...",
    "generatingVideo": "Generating video...",
    "generatingFollowUps": "Generating follow-up questions...",
    "sources": "Sources",
    "followUpTitle": "Follow-up questions",
    "callingTool": "Calling tool: {{toolName}}",
    "usedTool": "Used tool: {{toolName}}",
    "showDetails": "Show details",
    "stopGeneration": "Stop Generation",
    "tooltipAttachOrAnalyze": "Attach files or analyze image/video",
    "speechListening": "Listening...",
    "inputPlaceholder": "Ask me anything, or type 'generate image of...' or 'create video of...'",
    "searchChatsPlaceholder": "Search conversations...",
    "noChatsFound": "No chats found.",
    "sidebarFeatureInfo": "This feature is available in '{{feature}}' mode. Switch modes to see other content.",
    "model": "Model",
    "modelFlashLiteLabel": "Flash Lite",
    "modelFlashLiteDesc": "Fastest model for simple tasks.",
    "modelFlashLabel": "Flash",
    "modelFlashDesc": "Balanced speed and capability.",
    "modelProLabel": "Gemini 3 Pro",
    "modelProDesc": "Most capable for complex reasoning and deep analysis.",
    "tools": "Tools",
    "useKnowledgeBase": "Use Knowledge Base",
    "tooltipKnowledgeBase": "Allow the AI to use your uploaded documents to answer questions.",
    "searchWeb": "Search the web",
    "tooltipSearch": "Use Google Search for up-to-date information.",
    "useLocation": "Use current location",
    "tooltipLocation": "Provide location for relevant results (e.g., Maps).",
    "useClinic": "Clinic Finder",
    "tooltipClinic": "Allow the AI to find local health clinics.",
    "useSymptomChecker": "Symptom Checker",
    "tooltipSymptomChecker": "Allow the AI to analyze symptoms and provide general advice (not a diagnosis).",
    "useMedicationReminder": "Medication Reminder",
    "tooltipMedicationReminder": "Allow the AI to set single medication reminders using browser notifications.",
    "useMedicationScheduler": "Medication Scheduler",
    "tooltipMedicationScheduler": "Allow the AI to set up and manage recurring medication schedules.",
    "useAppointmentBooking": "Appointment Booker",
    "tooltipAppointmentBooking": "Allow the AI to book medical appointments.",
    "useUserProfile": "AI Memory",
    "tooltipUserProfile": "Allow the AI to remember details from your conversation for a personalized experience.",
    "useIsanDialect": "Speak Isan Dialect",
    "tooltipIsanDialect": "Make the AI speak in the local Isan dialect.",
    "personaModalTitle": "Set Custom Instructions",
    "personaModalBody": "Provide custom instructions for the AI to follow in its responses. This will apply to the current conversation.",
    "personaModalPlaceholder": "e.g., 'You are a friendly assistant who explains medical terms in simple language.'",
    "cancel": "Cancel",
    "save": "Save",
    "saving": "Saving...",
    "saveChanges": "Save Changes",
    "confirm": "Confirm",
    "apiKeyModalTitle": "API Key Required",
    "apiKeyModalBody1": "Video generation requires a user-provided API key.",
    "apiKeyModalBody2": "Please select a key to proceed. For information on billing, see the",
    "billingDocumentation": "billing documentation",
    "selectApiKey": "Select API Key",
    "liveConnecting": "Connecting to live session...",
    "liveErrorConnection": "Connection error. Please try again.",
    "liveErrorMic": "Could not access microphone.",
    "liveApiKeyErrorInitial": "This feature requires a user-provided API key with the 'AI Native Audio' API enabled.",
    "liveApiKeyErrorInvalid": "Invalid API Key. Please select a key with the 'AI Native Audio' API enabled.",
    "liveListening": "Listening... (Talk to the AI)",
    "liveSessionEnded": "Session ended.",
    "you": "You",
    "bot": "Bot",
    "liveStartCamera": "Start Camera",
    "liveStopCamera": "Stop Camera",
    "liveCaptureFrame": "Capture Frame",
    "liveCameraError": "Could not access camera. Please check permissions.",
    "emergencyModalTitle": "Emergency Assistance",
    "emergencyModalLoading": "Finding nearest emergency room...",
    "foundLocationTitle": "NEAREST 24/7 EMERGENCY ROOM:",
    "getDirections": "Get Directions",
    "emergencyNumbersTitle": "Emergency Hotlines",
    "emergencyNumberNarenthorn": "Narenthorn Center (Medical Emergency)",
    "emergencyNotifyContact": "Notify {{name}}",
    "emergencyNotifying": "Notifying...",
    "emergencyNotified": "Notified!",
    "emergencyContactNotified": "Emergency contact notified successfully.",
    "errorQuotaExceeded": "API quota exceeded. Please check your billing or try again later.",
    "errorImageSafety": "The request was blocked due to safety policies. Please adjust your prompt.",
    "voiceWakeWord": "assistant",
    "voiceSleepCommand": "go to sleep",
    "toastVoiceSleep": "Voice commands deactivated.",
    "toastVoiceProcessing": "Processing command...",
    "toastVoiceUnrecognized": "Sorry, I didn't understand that command.",
    "voiceCommandDescNewChat": "Starts a new, empty conversation or chat session.",
    "voiceCommandDescReadLastMessage": "Reads the last response from the AI assistant out loud.",
    "voiceCommandDescSetDarkMode": "Switches the user interface to dark mode or dark theme.",
    "voiceCommandDescSetLightMode": "Switches the user interface to light mode or light theme.",
    "voiceCommandDescToggleCamera": "Opens or closes the camera view. This is only available in the live conversation mode.",
    "voiceCommandDescAnalyzeImage": "Captures a frame from the camera and asks the AI to analyze it. This is only available in the live conversation mode when the camera is on.",
    "voiceCommandDescSwitchToLive": "Switches the user interface to the live conversation mode, usually to prepare for using the camera.",
    "sharedFooterTitle": "Experience the Public Health AI Assistant",
    "sharedFooterBody": "This is a shared conversation. Create your own account to start new chats, upload documents, and more.",
    "sharedFooterButton": "Try the AI Assistant Now",
    "analyzerTitle": "Health Report Analyzer",
    "analyzerSubtitle": "Upload an image of your lab report to get a simplified explanation of your results.",
    "analyzerUploadPrompt": "Drop your health report image here, or click to browse files.",
    "analyzerDisclaimerTitle": "Important Disclaimer",
    "analyzerDisclaimerBody": "This AI analysis is for informational purposes only and is NOT a substitute for professional medical advice, diagnosis, or treatment. Always consult with a qualified healthcare provider for any health concerns.",
    "analyzerButtonText": "Analyze Report",
    "analyzerLoading": "Analyzing report...",
    "analyzerError": "Could not analyze the report. The image might be unclear or the format may not be supported. Please try again with a clearer picture.",
    "analyzerErrorFileSize": "File is too large. Please select a file smaller than {{maxSize}}.",
    "analyzerErrorInvalidFile": "Invalid file type. Please upload an image.",
    "analyzerToastMultipleFiles": "Multiple files detected. Only the first file will be analyzed.",
    "analysisSummary": "Overall Summary",
    "analysisKeyFindings": "Key Findings",
    "analysisDetailedResults": "Detailed Breakdown",
    "statusHigh": "High",
    "statusLow": "Low",
    "statusNormal": "Normal",
    "statusAbnormal": "Abnormal",
    "hubTitle": "Health Hub",
    "hubSubtitle": "Latest health news and announcements for you, powered by AI.",
    "hubRefresh": "Refresh News",
    "hubLoading": "Finding latest health news...",
    "hubError": "Could not fetch health news. Please try again later.",
    "hubCategory_Alert": "Alert",
    "hubCategory_Campaign": "Campaign",
    "hubCategory_News": "News",
    "hubCategory_Tip": "Health Tip",
    "dashboardTitle": "Health Dashboard",
    "dashboardSubtitle": "Your personal health history. Track your analyzed reports over time.",
    "dashboardLoading": "Loading your health records...",
    "dashboardError": "Could not load health records. Please try again.",
    "dashboardEmptyTitle": "Your Dashboard is Empty",
    "dashboardEmptyBody": "After you analyze a health report in the 'Analyzer' tab, you can save it here to track your history.",
    "dashboardGoToAnalyzer": "Go to Analyzer",
    "dashboardSave": "Save to Dashboard",
    "dashboardSaving": "Saving...",
    "dashboardSavePrompt": "Enter a title for this report (e.g., Annual Checkup 2024):",
    "dashboardSaveSuccess": "Report saved to your dashboard!",
    "dashboardSaveError": "Failed to save the report.",
    "dashboardSaveDisabledTooltip": "Sign in to save reports to your dashboard.",
    "dashboardDeleteConfirm": "Are you sure you want to delete this health record? This action cannot be undone.",
    "dashboardDeleteSuccess": "Record deleted successfully.",
    "dashboardDeleteError": "Failed to delete the record.",
    "dashboardChartTitle": "Health Metric Trends",
    "dashboardChartSelectMetric": "Select a metric:",
    "dashboardChartMultiSelectTitle": "Select metrics to compare:",
    "dashboardChartNoData": "Not enough data to display a trend. At least two reports with the same metric are needed.",
    "dashboardChartNoMetrics": "No trackable metrics found in your saved reports.",
    "dashboardSummaryTitle": "Latest Health Vitals",
    "dashboardSummarySubtitle": "From your report on {{date}}",
    "dashboardNoVitals": "No key metrics found in your latest report to summarize.",
    "dashboardAiSummaryButton": "Generate AI Summary",
    "dashboardAiSummaryLoading": "Generating summary...",
    "dashboardAiSummaryTitle": "AI Health Summary",
    "dashboardAiSummaryError": "Could not generate an AI summary at this time.",
    "dashboardNutritionSummary": "Nutrition Snapshot",
    "dashboardWellnessSummary": "Wellness Check-in",
    "dashboardExportDoctor": "Export for Doctor",
    "dashboardExportDoctorSuccess": "Copied summary to clipboard!",
    "mapTitle": "Community Health Map",
    "mapSubtitle": "Find health services and information in your area.",
    "mapSearchPlaceholder": "Search for clinics, hospitals, or health alerts...",
    "mapSearchButton": "Search",
    "mapViewOnMap": "View on Map",
    "mapNoResults": "No results found for your query.",
    "mapError": "An error occurred while searching. Please try again.",
    "mapInitialMessage": "Use the search bar above to find health-related places and information near you.",
    "downloadFile": "Download {{fileName}}",
    "dashboardMedicationScheduleTitle": "Medication Schedule",
    "dashboardMedicationScheduleDosage": "Dosage",
    "dashboardMedicationScheduleFrequency": "Frequency",
    "dashboardMedicationScheduleTime": "Time",
    "dashboardMedicationScheduleEnds": "Ends on",
    "dashboardMedicationScheduleDeleteConfirm": "Are you sure you want to delete this medication schedule? All future reminders for it will be cancelled.",
    "profileTitle": "AI Memory Profile",
    "profileSubtitle": "Here is what the AI remembers about you. You can edit or remove any information.",
    "profileSectionPersonal": "Personal Information",
    "profileSectionHealth": "Health Information",
    "profileSectionEmergency": "Emergency Contact",
    "profileEmergencyName": "Name",
    "profileEmergencyPhone": "Phone Number",
    "profileSaveSuccess": "Profile updated successfully!",
    "profileSaveError": "Failed to update profile.",
    "profileSignInPromptTitle": "Sign In to Use Your Profile",
    "profileSignInPromptBody": "Sign in with your Google account to allow the AI to remember your preferences and create a personalized experience.",
    "nutritionTitle": "Nutrition Tracker",
    "nutritionSubtitle": "Track your daily meals and nutritional intake with AI.",
    "nutritionModePhoto": "Photo Mode",
    "nutritionModeText": "Text Mode",
    "nutritionUploadPrompt": "Drop a food photo here, or click to browse.",
    "nutritionTextPlaceholder": "Describe your meal (e.g., 'A bowl of chicken noodle soup with a side of toast').",
    "nutritionAnalyzeButton": "Analyze Nutrition",
    "nutritionSaveLog": "Save to Daily Log",
    "nutritionTodayLogs": "Today's Meals",
    "nutritionTotalCalories": "Total Calories",
    "nutritionNoLogs": "No meals logged today yet.",
    "wellnessTitle": "Wellness Center",
    "wellnessSubtitle": "Take a moment for your mental well-being. Check in with yourself.",
    "wellnessBreathingTitle": "Instant Calm: Box Breathing",
    "wellnessBreathingInstruction": "Breathe in... Hold... Breathe out... Hold...",
    "wellnessStartBreathing": "Start Exercise",
    "wellnessCheckInTitle": "How are you feeling today?",
    "wellnessMoodGreat": "Great",
    "wellnessMoodGood": "Good",
    "wellnessMoodOkay": "Okay",
    "wellnessMoodLow": "Low",
    "wellnessMoodStressed": "Stressed",
    "wellnessMoodAnxious": "Anxious",
    "wellnessNotePlaceholder": "Optional: What's on your mind?",
    "wellnessCheckInButton": "Check In & Get Support",
    "wellnessAiResponseTitle": "A message for you",
    "wellnessNewCheckIn": "New Check-in",
    "wellnessHistoryTitle": "Your Mood History",
    "wellnessTabMood": "Mood",
    "wellnessTabSleep": "Sleep",
    "sleepTrackerTitle": "Sleep Tracker",
    "sleepBedtime": "Bedtime",
    "sleepWaketime": "Wake Time",
    "sleepQuality": "Sleep Quality",
    "sleepQualityPoor": "Poor",
    "sleepQualityFair": "Fair",
    "sleepQualityGood": "Good",
    "sleepQualityExcellent": "Excellent",
    "sleepTrends": "Sleep Trends (Last 7 Days)",
    "saveLog": "Save Log",
    "sleepSavedSuccess": "Sleep log saved successfully.",
    "pharmacyTitle": "Smart Pharmacy",
    "pharmacySubtitle": "Identify medications and manage your medicine cabinet with AI.",
    "pharmacyTabScan": "Scan Medication",
    "pharmacyTabCabinet": "My Cabinet",
    "pharmacyUploadPrompt": "Take a photo of a pill, bottle, or package to identify it.",
    "pharmacyAnalyzeButton": "Identify Medication",
    "pharmacyLabelIndication": "What is it for?",
    "pharmacyLabelUsage": "How to take it:",
    "pharmacyLabelSideEffects": "Common Side Effects:",
    "pharmacyLabelWarnings": "Important Warnings:",
    "pharmacySafetySafe": "No conflicts detected",
    "pharmacySafetyWarning": "Potential Conflict Found",
    "pharmacyAddToCabinet": "Add to My Cabinet",
    "pharmacyAddedSuccess": "Medication added to your cabinet.",
    "pharmacyAlreadyExists": "This medication is already in your cabinet.",
    "pharmacyMyCabinetTitle": "My Medicines",
    "pharmacyCabinetEmpty": "Your cabinet is empty. Scan a medication to add it.",
    "elderlyModeTitle": "Elderly Mode",
    "elderlyModeExit": "Exit",
    "elderlyTalkToDoctor": "Talk to Doctor",
    "elderlyCallFamily": "Call Family",
    "elderlyFlashlight": "Flashlight",
    "queueTitle": "Mukdahan Hospital Queue",
    "queueStatusWaiting": "Waiting",
    "queueYourNumber": "Your Queue",
    "queueCurrent": "Current Queue",
    "queueWaitTime": "Approx. Wait",
    "queueMinutes": "min",
    "healthCoins": "Health Coins",
    "telemedDoctorName": "Dr. Somchai Jaidee",
    "telemedConnecting": "Connecting..."
  }
}

const translationsTh = {
  "translation": {
    "appTitle": "ผู้ช่วย AI สาธารณสุข",
    "welcomeTitle": "สวัสดี! มีอะไรให้ช่วยเหลือ?",
    "welcomeSubtitle": "ฉันคือผู้ช่วย AI จากสำนักงานสาธารณสุขจังหวัดมุกดาหาร",
    "welcomeUploadSupported": "ไฟล์ที่รองรับ: รูปภาพ, วิดีโอ, และเอกสาร (.pdf, .docx, .txt, etc.)",
    "suggestedQuestions": [
      "อาการของไข้เลือดออกมีอะไรบ้าง?",
      "ฉันปวดหัวและมีไข้ ควรทำอย่างไร?",
      "ช่วยนัดหมอฟันวันศุกร์หน้าตอนเช้าให้หน่อย",
      "ตั้งเวลาทานยาแก้แพ้ทุกวันตอนสองทุ่ม เป็นเวลา 30 วัน",
      "ฉันจะขอใบรับรองแพทย์ได้อย่างไร?",
      "สร้างแผนอาหารสุขภาพ 3 วันให้ฉันหน่อย",
      "ประเมินแคลอรี่จากรูปอาหารมื้อนี้ได้ไหม?"
    ],
    "capabilityTitle": "ความสามารถของฉัน",
    "capabilityChat": "สนทนาอัจฉริยะ",
    "capabilityChatDesc": "ถามคำถามที่ซับซ้อนได้",
    "capabilityImage": "วิเคราะห์รูปภาพ",
    "capabilityImageDesc": "วิเคราะห์และพูดคุยเกี่ยวกับรูปภาพ",
    "capabilityDoc": "ถาม-ตอบ ไฟล์",
    "capabilityDocDesc": "ถามคำถามเกี่ยวกับรูปภาพ, วิดีโอ, หรือเอกสารที่อัปโหลด",
    "capabilityFood": "วิเคราะห์อาหาร",
    "capabilityFoodDesc": "ประเมินแคลอรี่จากรูปถ่ายมื้ออาหาร",
    "capabilityVoice": "คำสั่งเสียง",
    "capabilityVoiceDesc": "ควบคุมแอปด้วยเสียงของคุณ",
    "recommendSystem": "แนะนำเมนูอาหารกลางวันเพื่อสุขภาพ",
    "recommendSystemPrompt": "แนะนำเมนูอาหารกลางวันเพื่อสุขภาพให้ฉันหน่อย",
    "imageGenerationTriggers": [
      "สร้างรูป",
      "สร้างภาพ",
      "วาดรูป"
    ],
    "imageEditingTriggers": [
      "แก้ไขรูป:",
      "แก้ไขภาพนี้:",
      "เปลี่ยนรูป:",
      "เปลี่ยนภาพนี้:",
      "เพิ่มในรูป:",
      "ลบจากรูป:"
    ],
    "videoGenerationTriggers": [
      "สร้างวิดีโอ",
      "ทำวิดีโอ"
    ],
    "newChatTitle": "แชทใหม่",
    "welcomeMessage": "สวัสดี! ฉันคือผู้ช่วย AI ด้านสาธารณสุข มีอะไรให้ช่วยเหลือ?",
    "locationError": "ไม่สามารถเข้าถึงตำแหน่งของคุณได้ กรุณาเปิดใช้งานบริการตำแหน่ง",
    "toastNoActiveConversation": "ไม่มีการสนทนาที่ใช้งานอยู่เพื่อส่งออก",
    "toastExporting": "กำลังส่งออกการสนทนา...",
    "docConfirmTitle": "เริ่มแชทใหม่ด้วยไฟล์นี้?",
    "docConfirmMessage": "การกระทำนี้จะสร้างการสนทนาใหม่จากไฟล์ '{{fileName}}'",
    "toastUnsupportedFile": "ไฟล์ '{{fileName}}' เป็นประเภทไฟล์ที่ไม่รองรับ",
    "toastUnsupportedFileForNewChat": "ไม่สามารถเริ่มแชทใหม่ได้ ประเภทไฟล์นี้ไม่รองรับ",
    "emergencyModalApiError": "ไม่สามารถติดต่อบริการฉุกเฉินได้ กรุณาตรวจสอบการเชื่อมต่อหรือใช้เบอร์โทรด้านล่าง",
    "emergencyModalLocationError": "ไม่สามารถระบุตำแหน่งของคุณสำหรับ SOS ได้ กรุณาเปิดบริการตำแหน่งแล้วลองอีกครั้ง",
    "toastNewChat": "เริ่มการสนทนาใหม่แล้ว",
    "toastReading": "กำลังอ่านข้อความล่าสุด...",
    "toastNoMessage": "ไม่มีข้อความให้อ่าน",
    "toastDarkMode": "เปลี่ยนเป็นโหมดมืด",
    "toastLightMode": "เปลี่ยนเป็นโหมดสว่าง",
    "toastCameraOn": "เปิดกล้องแล้ว",
    "toastCameraOff": "ปิดกล้องแล้ว",
    "toastAnalyzing": "กำลังวิเคราะห์ภาพ...",
    "toastNotInLiveMode": "คำสั่งนี้ใช้ได้เฉพาะในโหมดสนทนาสดเท่านั้น",
    "toastCameraNotOn": "กรุณาเปิดกล้องก่อน",
    "tooltipSos": "ฉุกเฉิน (SOS)",
    "chat": "แชท",
    "live": "สนทนาสด",
    "analyzer": "วิเคราะห์ผล",
    "hub": "ศูนย์ข้อมูล",
    "dashboard": "แดชบอร์ด",
    "map": "แผนที่",
    "profile": "โปรไฟล์",
    "nutrition": "โภชนาการ",
    "wellness": "สุขภาพใจ",
    "pharmacy": "เภสัชกร",
    "navGroupTalk": "สนทนา",
    "navGroupTools": "บริการ",
    "navGroupMe": "ส่วนตัว",
    "navGroupInfo": "ข้อมูล",
    "tooltipPersona": "ตั้งค่า Persona / คำสั่งระบบ",
    "tooltipExport": "ส่งออกการสนทนา",
    "tooltipTtsOn": "เปิดการอ่านออกเสียง",
    "tooltipTtsOff": "ปิดการอ่านออกเสียง",
    "tooltipVoiceOn": "คำสั่งเสียงกำลังทำงาน",
    "tooltipVoiceOff": "คำสั่งเสียงปิดอยู่",
    "tooltipLanguage": "เปลี่ยนภาษา",
    "tooltipTheme": "เปลี่ยนธีม",
    "tooltipFlashlight": "ไฟฉาย",
    "tooltipElderlyMode": "โหมดผู้สูงวัย",
    "copyCode": "คัดลอก",
    "errorOccurred": "เกิดข้อผิดพลาด",
    "clickToRetry": "คลิกเพื่อลองอีกครั้ง",
    "thinking": "กำลังคิด...",
    "generatingImage": "กำลังสร้างรูปภาพ...",
    "generatingVideo": "กำลังสร้างวิดีโอ...",
    "generatingFollowUps": "กำลังสร้างคำถามติดตาม...",
    "sources": "แหล่งข้อมูล",
    "followUpTitle": "คำถามที่อาจเกี่ยวข้อง",
    "callingTool": "กำลังเรียกใช้: {{toolName}}",
    "usedTool": "ใช้เครื่องมือ: {{toolName}}",
    "showDetails": "แสดงรายละเอียด",
    "stopGeneration": "หยุดการสร้าง",
    "tooltipAttachOrAnalyze": "แนบไฟล์หรือวิเคราะห์รูปภาพ/วิดีโอ",
    "speechListening": "กำลังฟัง...",
    "inputPlaceholder": "ถามอะไรก็ได้ หรือพิมพ์ 'สร้างรูป...' หรือ 'สร้างวิดีโอ...'",
    "searchChatsPlaceholder": "ค้นหาการสนทนา...",
    "noChatsFound": "ไม่พบการสนทนา",
    "sidebarFeatureInfo": "ประวัติการแชทจะแสดงในโหมดแชทเท่านั้น ขณะนี้คุณอยู่ในโหมด {{feature}}",
    "model": "โมเดล",
    "modelFlashLiteLabel": "แฟลช ไลท์",
    "modelFlashLiteDesc": "โมเดลที่เร็วที่สุดสำหรับงานง่ายๆ",
    "modelFlashLabel": "แฟลช",
    "modelFlashDesc": "ความเร็วและความสามารถที่สมดุล",
    "modelProLabel": "โปร",
    "modelProDesc": "ความสามารถสูงสุดสำหรับการให้เหตุผลที่ซับซ้อน",
    "tools": "เครื่องมืออำนวยความสะดวก",
    "useKnowledgeBase": "ใช้ฐานข้อมูลความรู้",
    "tooltipKnowledgeBase": "อนุญาตให้ AI ใช้เอกสารที่คุณอัปโหลดเพื่อตอบคำถาม",
    "searchWeb": "ค้นหาเว็บ",
    "tooltipSearch": "ใช้ Google Search เพื่อข้อมูลที่ทันสมัย",
    "useLocation": "ใช้ตำแหน่งปัจจุบัน",
    "tooltipLocation": "ให้ข้อมูลตำแหน่งเพื่อผลลัพธ์ที่เกี่ยวข้อง (เช่น แผนที่)",
    "useClinic": "ค้นหาคลินิก",
    "tooltipClinic": "อนุญาตให้ AI ค้นหาสถานพยาบาลในพื้นที่",
    "useSymptomChecker": "เครื่องมือตรวจสอบอาการ",
    "tooltipSymptomChecker": "อนุญาตให้ AI วิเคราะห์อาการและให้คำแนะนำทั่วไป (ไม่ใช่การวินิจฉัย)",
    "useMedicationReminder": "แจ้งเตือนการทานยา (ครั้งเดียว)",
    "tooltipMedicationReminder": "อนุญาตให้ AI ตั้งค่าการแจ้งเตือนการทานยาครั้งเดียวผ่านเบราว์เซอร์",
    "useMedicationScheduler": "จัดการตารางทานยา",
    "tooltipMedicationScheduler": "อนุญาตให้ AI ตั้งค่าและจัดการตารางการทานยาแบบประจำ",
    "useAppointmentBooking": "เครื่องมือช่วยนัดหมาย",
    "tooltipAppointmentBooking": "อนุญาตให้ AI ช่วยนัดหมายกับแพทย์",
    "useUserProfile": "หน่วยความจำ AI",
    "tooltipUserProfile": "อนุญาตให้ AI จดจำรายละเอียดจากการสนทนาของคุณเพื่อประสบการณ์ที่เป็นส่วนตัว",
    "useIsanDialect": "เว้าอีสาน (พูดภาษาถิ่น)",
    "tooltipIsanDialect": "ให้ AI พูดคุยตอบโต้ด้วยภาษาอีสาน",
    "personaModalTitle": "ตั้งค่าคำสั่งที่กำหนดเอง",
    "personaModalBody": "ระบุคำสั่งเพื่อให้ AI ปฏิบัติตามในการตอบสนอง ซึ่งจะใช้กับการสนทนาปัจจุบัน",
    "personaModalPlaceholder": "เช่น 'คุณคือผู้ช่วยที่เป็นมิตรซึ่งอธิบายศัพท์ทางการแพทย์ด้วยภาษาง่ายๆ'",
    "cancel": "ยกเลิก",
    "save": "บันทึก",
    "saving": "กำลังบันทึก...",
    "saveChanges": "บันทึกการเปลี่ยนแปลง",
    "confirm": "ยืนยัน",
    "apiKeyModalTitle": "ต้องการ API Key",
    "apiKeyModalBody1": "การสร้างวิดีโอต้องการ API Key ที่ผู้ใช้ระบุ",
    "apiKeyModalBody2": "กรุณาเลือก Key เพื่อดำเนินการต่อ สำหรับข้อมูลเกี่ยวกับการเรียกเก็บเงิน ดูที่",
    "billingDocumentation": "เอกสารการเรียกเก็บเงิน",
    "selectApiKey": "เลือก API Key",
    "liveConnecting": "กำลังเชื่อมต่อเซสชันสด...",
    "liveErrorConnection": "การเชื่อมต่อผิดพลาด กรุณาลองใหม่",
    "liveErrorMic": "ไม่สามารถเข้าถึงไมโครโฟนได้",
    "liveApiKeyErrorInitial": "ฟีเจอร์นี้ต้องการ API Key ที่ผู้ใช้ให้มา ซึ่งเปิดใช้งาน 'AI Native Audio' API แล้ว",
    "liveApiKeyErrorInvalid": "API Key ไม่ถูกต้อง กรุณาเลือก Key ที่เปิดใช้งาน 'AI Native Audio' API แล้ว",
    "liveListening": "กำลังฟัง... (พูดคุยกับ AI)",
    "liveSessionEnded": "เซสชันสิ้นสุดแล้ว",
    "you": "คุณ",
    "bot": "บอท",
    "liveStartCamera": "เปิดกล้อง",
    "liveStopCamera": "ปิดกล้อง",
    "liveCaptureFrame": "จับภาพ",
    "liveCameraError": "ไม่สามารถเข้าถึงกล้องได้ กรุณาตรวจสอบสิทธิ์การใช้งาน",
    "emergencyModalTitle": "ความช่วยเหลือฉุกเฉิน",
    "emergencyModalLoading": "กำลังค้นหาห้องฉุกเฉินที่ใกล้ที่สุด...",
    "foundLocationTitle": "ห้องฉุกเฉิน 24 ชั่วโมงที่ใกล้ที่สุด:",
    "getDirections": "ขอเส้นทาง",
    "emergencyNumbersTitle": "เบอร์โทรฉุกเฉิน",
    "emergencyNumberNarenthorn": "ศูนย์นเรนทร (การแพทย์ฉุกเฉิน)",
    "emergencyNotifyContact": "แจ้ง {{name}}",
    "emergencyNotifying": "กำลังแจ้งเตือน...",
    "emergencyNotified": "แจ้งเตือนแล้ว!",
    "emergencyContactNotified": "แจ้งผู้ติดต่อฉุกเฉินเรียบร้อยแล้ว",
    "errorQuotaExceeded": "โควต้า API เกินกำหนด กรุณาตรวจสอบการเรียกเก็บเงินของคุณหรือลองอีกครั้งในภายหลัง",
    "errorImageSafety": "คำขอถูกบล็อกเนื่องจากนโยบายความปลอดภัย กรุณาปรับเปลี่ยนข้อความของคุณ",
    "voiceWakeWord": "ผู้ช่วย",
    "voiceSleepCommand": "ไปนอน",
    "toastVoiceSleep": "ปิดใช้งานคำสั่งเสียงแล้ว",
    "toastVoiceProcessing": "กำลังประมวลผลคำสั่ง...",
    "toastVoiceUnrecognized": "ขออภัย ฉันไม่เข้าใจคำสั่งนั้น",
    "voiceCommandDescNewChat": "เริ่มการสนทนาหรือแชทใหม่ที่ว่างเปล่า",
    "voiceCommandDescReadLastMessage": "อ่านคำตอบล่าสุดจากผู้ช่วย AI ออกเสียง",
    "voiceCommandDescSetDarkMode": "เปลี่ยนส่วนติดต่อผู้ใช้เป็นโหมดมืดหรือธีมมืด",
    "voiceCommandDescSetLightMode": "เปลี่ยนส่วนติดต่อผู้ใช้เป็นโหมดสว่างหรือธีมสว่าง",
    "voiceCommandDescToggleCamera": "เปิดหรือปิดมุมมองกล้อง ใช้ได้เฉพาะในโหมดสนทนาสดเท่านั้น",
    "voiceCommandDescAnalyzeImage": "จับภาพจากกล้องและสั่งให้ AI วิเคราะห์ ใช้ได้เฉพาะในโหมดสนทนาสดเมื่อกล้องเปิดอยู่",
    "voiceCommandDescSwitchToLive": "เปลี่ยนหน้าจอผู้ใช้ไปยังโหมดสนทนาสด เพื่อเตรียมพร้อมใช้งานกล้อง",
    "sharedFooterTitle": "สัมผัสประสบการณ์ผู้ช่วย AI ด้านสาธารณสุข",
    "sharedFooterBody": "นี่คือการสนทนาที่ถูกแชร์ สร้างบัญชีของคุณเองเพื่อเริ่มแชทใหม่, อัปโหลดเอกสาร, และอื่นๆ",
    "sharedFooterButton": "ลองใช้ผู้ช่วย AI เลย",
    "analyzerTitle": "เครื่องมือวิเคราะห์ผลสุขภาพ",
    "analyzerSubtitle": "อัปโหลดรูปภาพผลตรวจจากห้องปฏิบัติการเพื่อรับคำอธิบายผลแบบง่ายๆ",
    "analyzerUploadPrompt": "วางไฟล์รูปภาพผลตรวจของคุณที่นี่ หรือคลิกเพื่อเลือกไฟล์",
    "analyzerDisclaimerTitle": "ข้อจำกัดความรับผิดชอบที่สำคัญ",
    "analyzerDisclaimerBody": "การวิเคราะห์โดย AI นี้มีวัตถุประสงค์เพื่อให้ข้อมูลเท่านั้น และไม่สามารถใช้แทนคำแนะนำ การวินิจฉัย หรือการรักษาทางการแพทย์จากผู้เชี่ยวชาญได้ โปรดปรึกษาผู้ให้บริการด้านสุขภาพที่มีคุณสมบัติเสมอสำหรับข้อกังวลด้านสุขภาพใดๆ",
    "analyzerButtonText": "วิเคราะห์ผล",
    "analyzerLoading": "กำลังวิเคราะห์ผล...",
    "analyzerError": "ไม่สามารถวิเคราะห์ผลตรวจได้ รูปภาพอาจไม่ชัดเจนหรือรูปแบบไม่รองรับ กรุณาลองอีกครั้งด้วยรูปที่ชัดเจนกว่า",
    "analyzerErrorFileSize": "ไฟล์มีขนาดใหญ่เกินไป กรุณาเลือกไฟล์ที่มีขนาดเล็กกว่า {{maxSize}}",
    "analyzerErrorInvalidFile": "ประเภทไฟล์ไม่ถูกต้อง กรุณาอัปโหลดไฟล์รูปภาพ",
    "analyzerToastMultipleFiles": "ตรวจพบหลายไฟล์ ระบบจะวิเคราะห์เฉพาะไฟล์แรกเท่านั้น",
    "analysisSummary": "สรุปภาพรวม",
    "analysisKeyFindings": "ผลที่น่าสนใจ",
    "analysisDetailedResults": "รายละเอียดผลการตรวจ",
    "statusHigh": "สูง",
    "statusLow": "ต่ำ",
    "statusNormal": "ปกติ",
    "statusAbnormal": "ผิดปกติ",
    "hubTitle": "ศูนย์ข้อมูลข่าวสาร",
    "hubSubtitle": "ข่าวสารและประกาศด้านสุขภาพล่าสุดสำหรับคุณ โดย AI",
    "hubRefresh": "รีเฟรชข่าว",
    "hubLoading": "กำลังค้นหาข่าวสุขภาพล่าสุด...",
    "hubError": "ไม่สามารถดึงข้อมูลข่าวสารได้ กรุณาลองอีกครั้งในภายหลัง",
    "hubCategory_Alert": "แจ้งเตือน",
    "hubCategory_Campaign": "รณรงค์",
    "hubCategory_News": "ข่าวสาร",
    "hubCategory_Tip": "เคล็ดลับสุขภาพ",
    "dashboardTitle": "แดชบอร์ดสุขภาพ",
    "dashboardSubtitle": "ประวัติสุขภาพส่วนตัวของคุณ ติดตามผลการวิเคราะห์ย้อนหลังได้ที่นี่",
    "dashboardLoading": "กำลังโหลดข้อมูลสุขภาพของคุณ...",
    "dashboardError": "ไม่สามารถโหลดข้อมูลสุขภาพได้ กรุณาลองอีกครั้ง",
    "dashboardEmptyTitle": "แดชบอร์ดของคุณว่างเปล่า",
    "dashboardEmptyBody": "หลังจากวิเคราะห์ผลตรวจในแท็บ 'วิเคราะห์ผล' คุณสามารถบันทึกมาไว้ที่นี่เพื่อติดตามประวัติได้",
    "dashboardGoToAnalyzer": "ไปที่หน้าวิเคราะห์ผล",
    "dashboardSave": "บันทึกลงแดชบอร์ด",
    "dashboardSaving": "กำลังบันทึก...",
    "dashboardSavePrompt": "ตั้งชื่อสำหรับผลตรวจนี้ (เช่น 'ตรวจสุขภาพประจำปี 2567'):",
    "dashboardSaveSuccess": "บันทึกผลตรวจลงแดชบอร์ดแล้ว!",
    "dashboardSaveError": "ไม่สามารถบันทึกผลตรวจได้",
    "dashboardSaveDisabledTooltip": "กรุณาลงชื่อเข้าใช้เพื่อบันทึกผลตรวจลงแดชบอร์ด",
    "dashboardDeleteConfirm": "คุณแน่ใจหรือไม่ว่าต้องการลบบันทึกสุขภาพนี้? การกระทำนี้ไม่สามารถย้อนกลับได้",
    "dashboardDeleteSuccess": "ลบบันทึกเรียบร้อยแล้ว",
    "dashboardDeleteError": "ไม่สามารถลบบันทึกได้",
    "dashboardChartTitle": "แนวโน้มข้อมูลสุขภาพ",
    "dashboardChartSelectMetric": "เลือกข้อมูล:",
    "dashboardChartMultiSelectTitle": "เลือกข้อมูลเพื่อเปรียบเทียบ:",
    "dashboardChartNoData": "ข้อมูลไม่เพียงพอที่จะแสดงแนวโน้ม ต้องมีรายงานอย่างน้อยสองฉบับที่มีข้อมูลเดียวกัน",
    "dashboardChartNoMetrics": "ไม่พบข้อมูลที่สามารถติดตามได้ในรายงานที่บันทึกไว้",
    "dashboardSummaryTitle": "ข้อมูลสุขภาพล่าสุด",
    "dashboardSummarySubtitle": "จากรายงานของคุณเมื่อวันที่ {{date}}",
    "dashboardNoVitals": "ไม่พบข้อมูลสำคัญในรายงานล่าสุดของคุณเพื่อสรุปผล",
    "dashboardAiSummaryButton": "สร้างสรุปด้วย AI",
    "dashboardAiSummaryLoading": "กำลังสร้างสรุป...",
    "dashboardAiSummaryTitle": "สรุปภาพรวมสุขภาพโดย AI",
    "dashboardAiSummaryError": "ไม่สามารถสร้างสรุปได้ในขณะนี้",
    "dashboardNutritionSummary": "โภชนาการวันนี้",
    "dashboardWellnessSummary": "สุขภาพใจล่าสุด",
    "dashboardAppointmentsTitle": "นัดหมายแพทย์",
    "dashboardTimelineTitle": "ไทม์ไลน์สุขภาพ",
    "dashboardNoAppointments": "ไม่มีนัดหมายเร็วๆ นี้",
    "dashboardExportDoctor": "สรุปข้อมูลเพื่อส่งแพทย์",
    "dashboardExportDoctorSuccess": "คัดลอกข้อมูลสรุปแล้ว!",
    "mapTitle": "แผนที่สุขภาพชุมชน",
    "mapSubtitle": "ค้นหาบริการและข้อมูลสุขภาพในพื้นที่ของคุณ",
    "mapSearchPlaceholder": "ค้นหาคลินิก, โรงพยาบาล, หรือประกาศเตือนภัย...",
    "mapSearchButton": "ค้นหา",
    "mapViewOnMap": "ดูบนแผนที่",
    "mapNoResults": "ไม่พบผลลัพธ์สำหรับคำค้นหาของคุณ",
    "mapError": "เกิดข้อผิดพลาดระหว่างการค้นหา กรุณาลองอีกครั้ง",
    "mapInitialMessage": "ใช้ช่องค้นหาด้านบนเพื่อค้นหาสถานที่และข้อมูลที่เกี่ยวกับสุขภาพใกล้คุณ",
    "downloadFile": "ดาวน์โหลด {{fileName}}",
    "dashboardMedicationScheduleTitle": "ตารางการทานยา",
    "dashboardMedicationScheduleDosage": "ปริมาณ",
    "dashboardMedicationScheduleFrequency": "ความถี่",
    "dashboardMedicationScheduleTime": "เวลา",
    "dashboardMedicationScheduleEnds": "สิ้นสุดวันที่",
    "dashboardMedicationScheduleDeleteConfirm": "คุณแน่ใจหรือไม่ว่าต้องการลบตารางยานี้? การแจ้งเตือนในอนาคตทั้งหมดจะถูกยกเลิก",
    "profileTitle": "โปรไฟล์หน่วยความจำ AI",
    "profileSubtitle": "นี่คือสิ่งที่ AI จดจำเกี่ยวกับคุณ คุณสามารถแก้ไขหรือลบข้อมูลใดก็ได้",
    "profileSectionPersonal": "ข้อมูลส่วนตัว",
    "profileSectionHealth": "ข้อมูลสุขภาพ",
    "profileSectionEmergency": "ผู้ติดต่อฉุกเฉิน",
    "profileEmergencyName": "ชื่อ",
    "profileEmergencyPhone": "เบอร์โทรศัพท์",
    "profileSaveSuccess": "อัปเดตโปรไฟล์เรียบร้อยแล้ว!",
    "profileSaveError": "ไม่สามารถอัปเดตโปรไฟล์ได้",
    "profileSignInPromptTitle": "ลงชื่อเข้าใช้เพื่อใช้โปรไฟล์ของคุณ",
    "profileSignInPromptBody": "ลงชื่อเข้าใช้ด้วยบัญชี Google เพื่อให้ AI จดจำค่ากำหนดของคุณและสร้างประสบการณ์ที่เป็นส่วนตัว",
    "nutritionTitle": "ตัวติดตามโภชนาการ",
    "nutritionSubtitle": "ติดตามมื้ออาหารและสารอาหารในแต่ละวันของคุณด้วย AI",
    "nutritionModePhoto": "โหมดรูปภาพ",
    "nutritionModeText": "โหมดข้อความ",
    "nutritionUploadPrompt": "วางรูปอาหารที่นี่ หรือคลิกเพื่อเลือกไฟล์",
    "nutritionTextPlaceholder": "อธิบายมื้ออาหารของคุณ (เช่น 'ก๋วยเตี๋ยวไก่ตุ๋น 1 ชาม กับกาแฟเย็น')",
    "nutritionAnalyzeButton": "วิเคราะห์โภชนาการ",
    "nutritionSaveLog": "บันทึกลงประวัติรายวัน",
    "nutritionTodayLogs": "อาหารที่กินวันนี้",
    "nutritionTotalCalories": "แคลอรี่รวม",
    "nutritionNoLogs": "ยังไม่มีการบันทึกอาหารในวันนี้",
    "wellnessTitle": "ศูนย์ดูแลสุขภาพใจ",
    "wellnessSubtitle": "พักสักนิด ดูแลจิตใจตัวเอง เช็คอินความรู้สึกของคุณ",
    "wellnessBreathingTitle": "ฝึกหายใจแบบกล่อง (Box Breathing)",
    "wellnessBreathingInstruction": "หายใจเข้า... กลั้น... หายใจออก... กลั้น...",
    "wellnessStartBreathing": "เริ่มฝึกหายใจ",
    "wellnessCheckInTitle": "วันนี้คุณรู้สึกอย่างไร?",
    "wellnessMoodGreat": "ยอดเยี่ยม",
    "wellnessMoodGood": "ดี",
    "wellnessMoodOkay": "เฉยๆ",
    "wellnessMoodLow": "แย่",
    "wellnessMoodStressed": "เครียด",
    "wellnessMoodAnxious": "กังวล",
    "wellnessNotePlaceholder": "เล่าเพิ่มเติมได้นะ (ไม่บังคับ)",
    "wellnessCheckInButton": "เช็คอิน & รับคำแนะนำ",
    "wellnessAiResponseTitle": "ข้อความถึงคุณ",
    "wellnessNewCheckIn": "เช็คอินใหม่อีกครั้ง",
    "wellnessHistoryTitle": "ประวัติอารมณ์ของคุณ",
    "wellnessTabMood": "อารมณ์",
    "wellnessTabSleep": "การนอนหลับ",
    "sleepTrackerTitle": "บันทึกการนอนหลับ",
    "sleepBedtime": "เข้านอน",
    "sleepWaketime": "ตื่นนอน",
    "sleepQuality": "คุณภาพการนอน",
    "sleepQualityPoor": "แย่",
    "sleepQualityFair": "พอใช้",
    "sleepQualityGood": "ดี",
    "sleepQualityExcellent": "เยี่ยมมาก",
    "sleepTrends": "สถิติการนอน (7 วันล่าสุด)",
    "saveLog": "บันทึกข้อมูล",
    "sleepSavedSuccess": "บันทึกข้อมูลการนอนเรียบร้อยแล้ว",
    "pharmacyTitle": "ห้องยาอัจฉริยะ",
    "pharmacySubtitle": "ระบุชนิดยาและจัดการตู้ยาของคุณด้วย AI",
    "pharmacyTabScan": "สแกนยา",
    "pharmacyTabCabinet": "ตู้ยาของฉัน",
    "pharmacyUploadPrompt": "ถ่ายรูปเม็ดยา ขวด หรือแผงยาเพื่อระบุชนิด",
    "pharmacyAnalyzeButton": "ระบุชนิดยา",
    "pharmacyLabelIndication": "สรรพคุณ:",
    "pharmacyLabelUsage": "วิธีใช้:",
    "pharmacyLabelSideEffects": "ผลข้างเคียงทั่วไป:",
    "pharmacyLabelWarnings": "คำเตือนสำคัญ:",
    "pharmacySafetySafe": "ไม่พบความขัดแย้ง",
    "pharmacySafetyWarning": "พบความเสี่ยงที่อาจเกิดขึ้น",
    "pharmacyAddToCabinet": "เพิ่มลงตู้ยา",
    "pharmacyAddedSuccess": "เพิ่มยาลงในตู้ยาแล้ว",
    "pharmacyAlreadyExists": "ยานี้มีอยู่ในตู้ยาแล้ว",
    "pharmacyMyCabinetTitle": "ยาของฉัน",
    "pharmacyCabinetEmpty": "ตู้ยาของคุณว่างเปล่า สแกนยาเพื่อเพิ่มรายการ",
    "elderlyModeTitle": "โหมดผู้สูงอายุ",
    "elderlyModeExit": "ออกจากโหมด",
    "elderlyTalkToDoctor": "คุยกับหมอ",
    "elderlyCallFamily": "โทรหาลูกหลาน",
    "elderlyFlashlight": "ไฟฉาย",
    "queueTitle": "สถานะคิว รพ.มุกดาหาร",
    "queueStatusWaiting": "รอรับบริการ",
    "queueYourNumber": "คิวของคุณ",
    "queueCurrent": "คิวปัจจุบัน",
    "queueWaitTime": "รออีกประมาณ",
    "queueMinutes": "นาที",
    "healthCoins": "เหรียญสุขภาพ",
    "telemedDoctorName": "นพ. สมชาย ใจดี",
    "telemedConnecting": "กำลังเชื่อมต่อ..."
  }
}

const translationsLo = {
  "translation": {
    "appTitle": "ຜູ້ຊ່ວຍ AI ສາທາລະນະສຸກ",
    "welcomeTitle": "ສະບາຍດີ! ມີຫຍັງໃຫ້ຊ່ວຍບໍ່?",
    "welcomeSubtitle": "ຂ້ອຍແມ່ນຜູ້ຊ່ວຍ AI ຈາກຫ້ອງການສາທາລະນະສຸກຈັງຫວັດມຸກດາຫານ",
    "welcomeUploadSupported": "ຮອງຮັບໄຟລ໌: ຮູບພາບ, ວິດີໂອ, ແລະ ເອກະສານ (.pdf, .docx, .txt, etc.)",
    "newChatTitle": "ແຊທ໌ໃໝ່",
    "welcomeMessage": "ສະບາຍດີ! ຂ້ອຍແມ່ນຜູ້ຊ່ວຍ AI ດ້ານສາທາລະນະສຸກຂອງເຈົ້າ. ມີຫຍັງໃຫ້ຊ່ວຍບໍ່?",
    "chat": "ແຊທ໌",
    "live": "ສົນທະນາສົດ",
    "analyzer": "ວິເຄາະຜົນ",
    "hub": "ສູນຂໍ້ມູນ",
    "dashboard": "ແດດຊ໌ບອດ",
    "map": "ແຜນທີ່",
    "profile": "ໂປຣໄຟລ໌",
    "nutrition": "ໂພຊະນາການ",
    "wellness": "ສຸຂະພາບຈິດ",
    "pharmacy": "ຮ້ານຂາຍຢາ",
    "navGroupTalk": "ສົນທະນາ",
    "navGroupTools": "ບໍລິການ",
    "navGroupMe": "ສ່ວນຕົວ",
    "navGroupInfo": "ຂໍ້ມູນ",
    "healthCoins": "ຫຼຽນສຸຂະພາບ",
    "elderlyModeTitle": "ໂຫມດຜູ້ສູງອາຍຸ",
    "elderlyModeExit": "ອອກຈາກໂຫມດ",
    "elderlyTalkToDoctor": "ລົມກັບຫມໍ",
    "elderlyCallFamily": "ໂທຫາລູກຫຼານ",
    "elderlyFlashlight": "ໄຟສາຍ",
    "queueTitle": "ສະຖານະຄິວ ໂຮງໝໍມຸກດາຫານ",
    "queueStatusWaiting": "ລໍຖ້າຮັບບໍລິການ",
    "queueYourNumber": "ຄິວຂອງທ່ານ",
    "queueCurrent": "ຄິວປັດຈຸບັນ",
    "queueWaitTime": "ລໍຖ້າປະມານ",
    "queueMinutes": "ນາທີ",
    "tooltipLanguage": "ປ່ຽນພາສາ",
    "telemedDoctorName": "ດຣ. ສົມຊາຍ ໃຈດີ",
    "telemedConnecting": "ກຳລັງເຊື່ອມຕໍ່...",
    "dashboardTitle": "ແດດຊ໌ບອດສຸຂະພາບ",
    "dashboardSubtitle": "ປະຫວັດສຸຂະພາບສ່ວນຕົວຂອງທ່ານ",
    "dashboardAiSummaryButton": "ສ້າງບົດສະຫຼຸບ AI",
    "dashboardNutritionSummary": "ໂພຊະນາການວັນນີ້",
    "dashboardWellnessSummary": "ສຸຂະພາບຈິດຫຼ້າສຸດ",
    "dashboardAppointmentsTitle": "ນັດຫມາຍແພດ",
    "dashboardTimelineTitle": "ໄທມ໌ລາຍສຸຂະພາບ"
  }
};

i18next
  .use(initReactI18next)
  .init({
    resources: {
      en: translationsEn,
      th: translationsTh,
      lo: translationsLo,
    },
    lng: 'th', // default language
    fallbackLng: 'en',
    interpolation: {
      escapeValue: false, // react already safes from xss
    },
  });

type Language = 'en' | 'th' | 'lo';

interface LanguageContextType {
  language: Language;
  setLanguage: (language: Language) => void;
  // @google/genai-fix: Correctly type the 't' function.
  t: TFunction;
}

const LanguageContext = createContext<LanguageContextType | undefined>(undefined);

export const LanguageProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  const { t, i18n } = useI18nextTranslation();
  const [language, setLanguageState] = useState<Language>((i18n.language as Language) || 'th');

  const setLanguage = useCallback((lang: Language) => {
    i18n.changeLanguage(lang);
    setLanguageState(lang);
  }, [i18n]);

  const value = { language, setLanguage, t };

  return React.createElement(LanguageContext.Provider, { value: value }, children);
};

export const useTranslation = (): LanguageContextType => {
  const context = useContext(LanguageContext);
  if (context === undefined) {
    throw new Error('useTranslation must be used within a LanguageProvider');
  }
  return context;
};
